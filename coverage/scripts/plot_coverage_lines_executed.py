...
output_plot = "../graphs/lines_executed.png"

# Collect all "*_lines_executed.csv" files in "csv_files/Test x" folders
csv_files = []
for root, dirs, files in os.walk(summary_base_dir):
    for file in files:
        if file.endswith("_lines_executed.csv"):
...
if 'Lines Executed %' not in all_data.columns:
    raise ValueError("'Lines Executed %' column not found in the data.")

# Filter relevant data
lines_data = all_data[['Filename', 'Test Folder', 'Lines Executed %']].copy()

# Calculate min and max values to find ranges
value_ranges = lines_data.groupby('Filename')['Lines Executed %'].agg(['min', 'max'])
value_ranges['range'] = value_ranges['max'] - value_ranges['min']

# Filter files with changes in 'Lines Executed %'
...
if lines_data_filtered.empty:
    raise ValueError("No changes detected in 'Lines Executed %' across files.")
...
# Customize plot
plt.title("Changes in Lines Executed % Across Test Folders (Filtered for Changes)")
plt.xlabel("Test Folder")
plt.ylabel("Lines Executed % (%)")
